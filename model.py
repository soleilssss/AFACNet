...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1,filter_size=3):
        super().__init__()
        ...
        if stride != 1:
            self.residual_function = Sequential_complex(
            ComplexLowpass(filt_size=filter_size, stride=1, channels=out_channels),
            ComplexConv2d(out_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False),
            ComplexBatchNorm2d(out_channels),
            ComplexReLU(inplace=True),  #inplace = true
            ComplexConv2d(out_channels, out_channels * BasicBlock.expansion, kernel_size=3, padding=1, bias=False),
            ComplexBatchNorm2d(out_channels * BasicBlock.expansion)
        ...
        if stride != 1 or in_channels != BasicBlock.expansion * out_channels:
            self.shortcut = Sequential_complex(
                ComplexLowpass(filt_size=filter_size, stride=stride, channels=in_channels),
                ComplexConv2d(in_channels, out_channels * BasicBlock.expansion, kernel_size=1, stride=1, bias=False),
                ComplexBatchNorm2d(out_channels * BasicBlock.expansion)
            )

    def forward(self, x1, x2):
        x3, x4 = self.residual_function(x1, x2) 
        # x5, x6 = self.shortcut(x1, x2)
        # x1, x2 = x3 + x5, x4 + x6 
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=2, inputchannel=1, filter_size=3):
        super().__init__()
        ...
        self.maxpool = ComplexMaxPool2d(kernel_size=3, stride=1, padding=1)
        self.lowpass = ComplexLowpass(self.in_channels, filt_size=filter_size, stride=2)
        #we use a different inputsize than the original paper
        #so conv2_x's stride is 1
        self.conv2_x = self._make_layer(block, 64, num_block[0], 1, filter_size=filter_size)
        self.conv3_x = self._make_layer(block, 128, num_block[1], 2, filter_size=filter_size)
        self.conv4_x = self._make_layer(block, 256, num_block[2], 2, filter_size=filter_size)
        self.conv5_x = self._make_layer(block, 512, num_block[3], 2, filter_size=filter_size)
        self.avg_pool = Complex_AdaptiveAvgPool2d((1, 1))
        self.fc = ComplexLinear(512 * block.expansion, num_classes)

    def _make_layer(self, block, out_channels, num_blocks, stride, filter_size):
...
def resnet18(num_classes=4, inputchannel=3, filter_size=3):
    """ return a ResNet 18 object
    """
    return ResNet(BasicBlock, [2, 2, 2, 2],num_classes=4, inputchannel=3, filter_size=3)
